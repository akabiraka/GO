...
def generate_dataset(GOname="BP", GO_terms_set=bp_set, cutoff_value=125, atleast_n_annots=0):
    dev_set = {} # uniprot_id, set of annots
    ...
    for i, line in enumerate(f.readlines()):
        ...
        if GO_id in GO_terms_set and date<=t1:
            dev_set = update_annot_dict(uniprot_id, GO_id, dev_set)
    
    print(f"#-prots in dev_set: {len(dev_set)}")
    ...
    train_set, test_set = train_test_split(list(dev_set.items()), test_size=0.15, random_state=123)
    train_set, val_set = train_test_split(train_set, test_size=0.15, random_state=123)
